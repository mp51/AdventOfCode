...
def spread_oxygen(tiles_map, print_=False):
   def is_adjacent_to_oxygen(i, j):
      if tiles_map[i][j] == '.' and \
         (tiles_map[i+1][j] == '*' \
            or tiles_map[i-1][j] == '*' \
               or tiles_map[i][j+1] == '*' \
                  or tiles_map[i][j-1] == '*'):
   ...
   while True:
      # get border of oxygen-filled tiles
      tiles_to_fill = []
      for i in range(1, len(tiles_map)-1):
         for j in range(1, len(tiles_map[0])-1):
            if is_adjacent_to_oxygen(i,j):
               tiles_to_fill.append( (i, j) )
      
      if not tiles_to_fill:
         break
...
# Oxygen spreading
t = spread_oxygen(tiles_map)
print("Oxygen filled in", t, "minutes")